...
def application(environ, start_response):
    ...
    if way == 'post':
        ...
        elif reg(re.match('i=(\d)+&j=(\d+)&alu$', raw)):
            o, mime = display(int(reg.v.group(1)), int(reg.v.group(2)), True), 'image/svg+xml'
        else:
            o = 'YES'
            oo, mime = display(), 'image/svg+xml'
...
def display(nx=5, ny=10, alu=False):
    qrs = 201505180000
    o, k = '<svg %s width="%s" height="%s">\n' % (_SVGNS, 50+380*nx, 50+440*ny), 0
    o += '<style type="text/css">@import url(http://fonts.googleapis.com/css?family=Schoolbell);@import url(http://fonts.googleapis.com/css?family=Varela+Round);h1,h2,p,li,i,b,a,div,input,td,th,footer,svg{font-family:"Lucida Grande", "Lucida Sans Unicode", Helvetica, Arial, Verdana, sans-serif}a.mono,p.mono,td.mono,b.mono{font-family:"Lucida Concole",Courier;font-weight:bold}text.alpha{font-family:Varela Round}</style>'
    o += '<text font-family="arial" font-size="9" x="20" y="20">%d colonnes %d lignes (angle:30 deg)</text>' % (nx, ny)
    for j in range(ny):
        for i in range(nx):
            k += 1
            s = '%d' % (qrs+k)
            o += QRCode(s, 2).svg(100+380*i, 150+440*j, 10, s, alu) 
    return o + '</svg>\n'
